...
ただし、以下は事前に準備してください。
GitHubアカウントの作成・Node.jsのインストール・ElevenLabsアカウントの作成・APIキーの取得については、
AIコーディングツールに「〇〇の方法を教えてください」と聞きながら進めるのがおすすめです。

- GitHubアカウントの作成（AIに聞きながら進めてOK）
- Node.jsのインストール（AIに聞きながら進めてOK）
- ElevenLabsアカウントの作成（AIに聞きながら進めてOK）
- ElevenLabs APIキーの取得（AIに聞きながら進めてOK）
- リポジトリのFork、またはダウンロード（下記の手順を参照）
...
## ZIP版で使う場合（GitHubが難しい方向け）

GitHubの操作が難しい場合は、ZIPファイルを解凍して使うこともできます。

### 手順

1. ZIPファイルをダウンロードする
2. ZIPを解凍する
3. 解凍したフォルダをAntigravity、VS Code、またはCursorなどで開く
4. AIに「READMEを読んで、このプロジェクトをセットアップしてください」と依頼する
5. `.env.example` をコピーして `.env` を作る
6. `.env` に自分のElevenLabs APIキーを入力する
7. `npm install` を実行する
8. `npm start` で起動する

### 効果音（SFX）の生成について

ZIPには効果音ファイルが含まれていません。セットアップ後、以下のコマンドで生成してください。

```bash
node scripts/generate-sfx.js
```

ElevenLabs APIキーが設定済みであれば、自動的に効果音が生成されます。

### 注意

- `.env` に入力したAPIキーは絶対に他人に見せないでください
- Node.jsのインストールとElevenLabs APIキーの取得は必要です
- ZIPを更新版に差し替えたい場合は、新しいZIPを再度受け取って解凍し直してください
