...
def generate_image(scene_id, request_data):
    try:
        ...
        for attempt in range(3):
            try:
                response = client.models.generate_content(
                    model='gemini-2.0-flash', # Trying Flash for speed/availability. Or 'gemini-3-pro-image-preview'
                ...
                if response.candidates and response.candidates[0].content.parts:
                    for part in response.candidates[0].content.parts:
                        if part.inline_data:
                            ...
                            return True
                    print(f"  Failed: No inline data in response.")
                else:
                ...
                if "429" in str(e):
                    ...
                    # Try removing reference image if 403 (sometimes works?)
                    # Or try creating a simpler prompt?
                    pass
                time.sleep(2)
...
def main():
    ...
    for scene_id in missing_ids:
        if scene_id in requests_map:
            if generate_image(scene_id, requests_map[scene_id]):
                count += 1
